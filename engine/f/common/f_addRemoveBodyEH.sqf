...
{
	_str_men = str _men;
	player sideChat format ["DEBUG (engine\f\common\f_addRemoveBodyEH.sqf): _men = %1",_str_men];
};
...
if (f_var_debugMode == 1) then
{
	...
	player sideChat format ["DEBUG (engine\f\common\f_addRemoveBodyEH.sqf): _menLessExempt = %1",_str_menLessExempt];
};
...
}else
{
	...
	if (f_var_debugMode == 1) then
	{
	player sideChat "DEBUG (engine\f\common\f_addRemoveBodyEH.sqf): FIFO EHs added";
	};
};
