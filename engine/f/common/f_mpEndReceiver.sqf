...
case 1:
{
      titleText [format ["  "], "PLAIN DOWN",1]; // clear the screen from any messages
        if (isServer) then {
            ["ending_assessment", [1]] call CBA_fnc_localEvent;
        };        
        // We then wait a bit so that the players can actually read it.
        sleep _waitTime;
        vehicle player enableSimulation true;
        // And then end the mission with the desired ending.
        endMission "END1";		
};
