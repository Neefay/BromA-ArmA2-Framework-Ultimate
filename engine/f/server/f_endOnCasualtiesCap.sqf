...
{
	player sideChat format ["DEBUG (engine\f\server\f_endOnCasualtiesCap.sqf): _grpstemp = %1",_grpstemp];
};
...
if (f_var_debugMode == 1) then
{
	player sideChat format ["DEBUG (engine\f\server\f_endOnCasualtiesCap.sqf): _grps = %1",_grps];
};
...
if (f_var_debugMode == 1) then
{
	player sideChat format ["DEBUG (engine\f\server\f_endOnCasualtiesCap.sqf): _started = %1",_started];
};
...
// DEBUG
	if (f_var_debugMode == 1) then
	{
		player sideChat format ["DEBUG (engine\f\server\f_endOnCasualtiesCap.sqf): _remaining = %1",_remaining];
	};
